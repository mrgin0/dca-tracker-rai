...
export const state = {
  ...
  editId: null,        // id transaksi yang sedang diubah (modal edit)
  editAsset: null,     // simbol aset dari transaksi tsb
  data: {},
  user: null,
  branding: null,
...
/**
 * Semua simbol yang relevan: instrumen aktif + simbol yang masih punya
 * transaksi walaupun instrumennya sudah dihapus (supaya totalnya tidak bocor).
 */
export function allSymbols() {
  const set = new Set(state.assets.map((a) => a.symbol));
  Object.keys(state.data || {}).forEach((s) => {
    if ((state.data[s] || []).length > 0) set.add(s);
  });
  return [...set];
}

/** Daftar transaksi sebuah simbol, selalu berupa array. */
export function entriesOf(symbol) {
  const list = state.data?.[symbol];
  return Array.isArray(list) ? list : [];
}

export function colorForAsset(symbol) {
  if (ASSET_COLORS[symbol]) return ASSET_COLORS[symbol];
  const custom = allSymbols().filter((s) => !ASSET_COLORS[s]);
